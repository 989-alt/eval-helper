...
export function renderGyogwa(root) {
  ...
  function syncStandardsFromDOM() {
    ...
    inputs.forEach((inp, i) => { if (i < standards.length) standards[i] = inp.value; });
  }
  ...
  function renderStandards() {
    mount(stdWrap, ...standards.map((val, i) => {
      const inp = h('input', {
        ...
        placeholder: '예: 비와 비율 / 토론에서 근거 들어 주장하기',
        value: val,
      });
      inp.addEventListener('input', () => { standards[i] = inp.value; });
      const row = h('div', { class: 'flex items-center gap-2' }, inp);
      if (standards.length > 1) {
        const delBtn = h('button', { class: 'text-red-500 hover:text-red-700 px-2 font-bold' }, '×');
        delBtn.addEventListener('click', () => {
          syncStandardsFromDOM();
  ...
  const addStdBtn = h('button', {
    class: 'text-sm bg-blue-100 text-blue-700 px-3 py-1 rounded hover:bg-blue-200 font-bold transition',
  }, '+ 추가');
  ...
  const countRow = h('div', { class: 'grid grid-cols-2 sm:grid-cols-4 gap-3' },
    ...LEVELS.map((lv) => {
      const inp = h('input', {
        ...
        type: 'number', min: '0', max: '40', value: '3',
      });
      countInputs[lv] = inp;
  ...
  function readCounts() {
    const c = {};
    LEVELS.forEach((lv) => { c[lv] = Math.max(0, Math.min(40, parseInt(countInputs[lv].value) || 0)); });
    return c;
  }

  function readStandards() {
    syncStandardsFromDOM();
    return standards.map((s) => s.trim()).filter(Boolean);
  }

  function showBanks(results) {
    if (!results.length) {
      mount(bankOut, h('div', { class: css.card }, notice('생성된 평어가 없습니다.', 'warn')));
      return;
    }
    let activeTab = 0;
    ...
    function renderPanel() {
      ...
      const { bank } = r;
      const chip = h('div', {
        class: 'bg-blue-50 p-3 rounded border border-blue-100 mb-4 text-sm text-blue-900 font-medium',
      }, r.standard);
      ...
      mount(tabBar, ...results.map((r, i) => {
        const btn = h('button', {
          class: i === activeTab
            ...
            : 'px-5 py-3 text-sm font-bold whitespace-nowrap text-gray-500 hover:bg-gray-100',
        }, '성취기준 ' + (i + 1));
  ...
  genBtn.addEventListener('click', () => {
    ...
    const results = names.map((name) => ({ standard: name, bank: generateBank(name, counts) }));
    showBanks(results);
  });

  const aiBtn = h('button', {
    class: 'px-5 py-3 rounded-lg font-bold border border-blue-200 text-blue-700 hover:bg-blue-50 transition whitespace-nowrap disabled:opacity-50',
  }, 'AI로 생성');
  aiBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      for (const name of names) {
        results.push({ standard: name, bank: await aiBank(ai, name, counts) });
      }
      showBanks(results);
    } catch (e) {
      mount(bankOut, notice('AI 생성 실패: ' + (e?.message || e), 'warn'));
    } finally {
      aiBtn.textContent = 'AI로 생성'; aiBtn.disabled = false;
    }
  });
  ...
  function syncCritFromDOM() {
    ...
    inputs.forEach((inp, i) => { if (i < critCounts.length) critCounts[i] = Math.max(1, parseInt(inp.value) || 1); });
  }

  function renderCrit() {
    mount(critWrap, ...critCounts.map((val, i) => {
      const inp = h('input', {
        class: css.input + ' w-20 text-right',
  ...
  const numStudentsInp = h('input', {
    class: css.input + ' w-24',
    type: 'number', min: '1', max: '50', value: '25',
  });

  const numSubjectsInp = h('input', {
    class: css.input + ' w-24',
  ...
  const tplBtn = h('button', {
    class: 'text-sm bg-teal-50 text-teal-700 border border-teal-200 px-4 py-2 rounded hover:bg-teal-100 font-bold transition',
  }, '엑셀 양식 다운로드');
  tplBtn.addEventListener('click', () => {
    syncCritFromDOM();
    const N = Math.max(1, Math.min(50, parseInt(numStudentsInp.value) || 1));
    try {
      const skeleton = buildSkeletonEvalSet(N, critCounts);
      downloadTemplate(skeleton, '교과평어_양식.xlsx');
  ...
  fileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      const rows = generatePerStudent(parsed);
      showStudentResults(rows);
    } catch (e) { toast('엑셀 읽기 실패: ' + (e?.message || e)); }
  });

  function showStudentResults(rows) {
    if (!rows.length) {
      mount(xlsOut, notice('생성할 점수가 없습니다. 엑셀에 성취수준을 입력했는지 확인하세요.', 'warn'));
      return;
    }
    ...
    mount(xlsOut, h('div', { class: css.card + ' animate-fade-in' },
      h('div', { class: 'flex flex-wrap items-center justify-between gap-2 mb-4' },
        ...
        h('div', { class: 'flex gap-2' },
          h('button', {
            class: 'text-sm bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700 font-bold shadow transition',
            onClick: () => exportDoc(byStudent),
          }, '💾 한글(.doc)'),
          h('button', {
            class: 'text-sm bg-white text-gray-600 border border-gray-200 px-3 py-2 rounded hover:bg-gray-50 font-bold transition',
            onClick: () => downloadText(rows.map((r) => `${r.studentName}\t${r.subjectName}\t${r.text}`).join('\n'), '평어.txt'),
          }, '텍스트'),
        ),
      ),
  ...
  function exportDoc(byStudent) {
    const sections = [...byStudent.entries()].map(([name, items]) => ({
      heading: name,
      lines: items.map((r) => ({ label: r.subjectName, text: r.text })),
    }));
  ...
  renderStandards();
  renderCrit();
  mount(root, h('div', { class: 'animate-fade-in' }, cardA, bankOut, cardB, xlsOut));
}
